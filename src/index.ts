...
app.get("/", async (req, res) => {
  const user: Presence = await getUser(process.env.USERID);
...
const getUser = async (discordUserId: string): Promise<Presence> => {
  const list: Guild = await client.guilds.fetch(process.env.GUILDID);
  const members: GuildMember = await list.members.fetch(discordUserId);
